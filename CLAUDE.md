...
## CRM-интеграция (управление комиссиями) — ИЗОЛИРОВАННЫЙ модуль

Сервис умеет **создавать, изменять и удалять** кастомные комиссии (техкост) на
сущности в CRM (`newcrm.hybrid.ai`). UI: вкладка «Pace» (кнопка «💰 техкост» на
green-строках) + вкладка «Комиссии» (реестр созданных, изменить/удалить).
...
**Два предохранителя:**
- `CRM_ENABLED=false` — endpoint'ы отдают 403, кнопок/вкладок в UI нет
- `CRM_DRY_RUN=true` — включённый модуль только логирует payload, НЕ шлёт в CRM

**Endpoint'ы CRM (`https://newcrm.hybrid.ai/core/AgencyCommission/`):**

| Endpoint | Что |
|---|---|
| `POST .../Create` | Создать комиссию. `Commission.Id=null`. Тело-ответ пустое (id не возвращает). |
| `POST .../Update` | Изменить. `Commission.Id` = mongo-id комиссии. |
| `DELETE .../Delete?id=<mongo-id>` | Удалить. Успех = 204. |
| `POST .../GetCampaignCommission` | Комиссии кампаний. Фильтр работает! |
| `POST .../GetAdvertiserCommission` | Комиссии рекламодателей. |
| `POST .../GetCommission` | Только agency-level комиссии. Фильтры в теле НЕ работают. |

**Уровни комиссии** — определяются набором полей в `CommissionEntity`:
- `agency` — `{AgencyId, AdvertiserId:null, TradingDeskId}`
- `advertiser` — `{AgencyId, AdvertiserId, TradingDeskId}`
- `campaign` — `{AgencyId, AdvertiserId, FolderId:null, CampaignId, TradingDeskId}`

**Резолв id созданной комиссии** (Create id не возвращает):
- После Create дёргаем `GetCampaignCommission` (для campaign) / `GetAdvertiserCommission`
  (advertiser) / `GetCommission` (agency).
- Фильтр в теле: `AgencyId` = **inventoryId агентства** (НЕ id! CRM хочет inventoryId),
  `AdvertiserId` = обычный advertiser id. У `GetCampaignCommission`/`GetAdvertiserCommission`
  фильтр работает точечно (count=1), у `GetCommission` — нет.
- В ответе матчим по `entityId`: для campaign `entityId == campaign_id`,
  для agency `entityId == agency.inventoryId`.
- Берём `id` (mongo, для Update/Delete) и `overallId` (числовой id из интерфейса CRM).

**Нюансы (выстраданы на боевых тестах):**
- `CommissionScale` — **только целое** (int). Дробный float ломает десериализацию
  всего тела → 400. Шлём `int(round(scale))`.
- `TradingDeskId` — один на кабинет, из `CRM_TRADING_DESK_IDS` по tenant'у.
- CRM **флапает 401** (csid нестабилен). На 401 авто-ретрай до 3 раз — безопасно:
  401 = запрос не прошёл, дубля не будет. На 5xx/таймаут НЕ ретраим.
- `agency.inventoryId` (из `GetSelfAccounts`) прокинут через `CampaignPaceOut`
  как `agency_inventory_id` — нужен для фильтров резолва.
- `commissions_registry.json` — реестр созданных через сервис комиссий (изоляция:
  вкладка «Комиссии» показывает только их, не лезет за всем списком CRM).
- `commissions_log.json` — лог всех операций (dry-run и боевых).

**Статус:** полный цикл проверен на боевой campaign-level комиссии —
create (200) → update (200) → delete (204). Резолв id для **agency/advertiser**
уровней написан по аналогии, но на боевых НЕ проверен.
...
app/
  ...
  crm.py            # ИЗОЛИРОВАННЫЙ CRM-модуль: APIRouter /crm/*, create/update/
                    # delete комиссий, реестр, резолв id по уровням
  static/
    index.html      # UI (vanilla JS + CSS): вкладки Pace/Комиссии, таблица,
                    # фильтры, sync progress, auth alert, CRM модалка create/edit
...
- `POST /crm/commission/update` — изменить размер комиссии по её id
- `DELETE /crm/commission/{id}` — удалить комиссию
- `GET /crm/commissions` — реестр комиссий созданных через сервис (для вкладки)
- `GET /crm/status` — состояние CRM-модуля (enabled/dry_run/td-map)
- `GET /crm/log` — лог всех CRM-операций
- `GET /docs` — Swagger
...
7. **CRM: защита от дублей** — повторное создание комиссии на одной кампании может
   ...
   `commissions_registry.json` и предупреждать «уже создавали».
...
9. **CRM: проверить резолв id для agency/advertiser уровней** — campaign-level
   проверен на боевых, agency/advertiser написаны по аналогии но не тестировались.
...
- **Резолв id комиссии** — Create id не возвращает, поэтому после создания дёргаем `GetCampaignCommission`/`GetAdvertiserCommission` (фильтр там работает, в отличие от `GetCommission`), матчим по `entityId`. Фильтр `AgencyId` хочет inventoryId агентства, не id.
- **Изоляция CRM по данным** — вкладка «Комиссии» рисуется из `commissions_registry.json` (наши комиссии), сервис не выгружает чужие комиссии из CRM.
